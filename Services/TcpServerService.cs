...
namespace SteamCmdWebAPI.Services
{
    public class TcpClientService
    {
        ...
        public async Task<bool> TestConnectionAsync(string serverAddress = null, int port = 0)
        {
        ...
        public async Task<List<string>> GetProfileNamesAsync(string serverAddress = null, int port = 0)
        {
            // Luôn sử dụng địa chỉ mặc định bất kể tham số đầu vào
            serverAddress = DEFAULT_SERVER_ADDRESS;
            port = DEFAULT_SERVER_PORT;
        ...
        public async Task<SteamCmdProfile> GetProfileDetailsByNameAsync(string profileName, string serverAddress = null, int port = 0)
        {
            // Luôn sử dụng địa chỉ mặc định bất kể tham số đầu vào
            serverAddress = DEFAULT_SERVER_ADDRESS;
            port = DEFAULT_SERVER_PORT;
            ...
            try
            {
                ...
                using (var tcpClient = new TcpClient())
                {
                    ...
                    try
                    {
                        var profile = JsonSerializer.Deserialize<SteamCmdProfile>(response);
        ...
        public async Task<bool> SendProfileToServerAsync(SteamCmdProfile profile)
        {
            ...
            {
                _logger.LogError(ex, "Lỗi khi gửi profile {ProfileName} lên server", profile?.Name ?? "unknown");
                return false;
            }
        }
        ...
        public async Task<bool> SendMultipleProfilesToServerAsync(List<SteamCmdProfile> profiles)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Đang gửi {Count} profiles lên server {Server}:{Port}",
                    profiles.Count, serverAddress, port);
                ...
                using (var tcpClient = new TcpClient())
                {
                    ...
                    string command = $"AUTH:{AUTH_TOKEN} SEND_PROFILES";
                    byte[] commandBytes = Encoding.UTF8.GetBytes(command);
                    byte[] lengthBytes = BitConverter.GetBytes(commandBytes.Length);
                    ...
                    // Gửi marker kết thúc - độ dài bằng 0
                    byte[] zeroLengthBytes = BitConverter.GetBytes(0);
                    await stream.WriteAsync(zeroLengthBytes, 0, zeroLengthBytes.Length);
                    await stream.FlushAsync();

                    // Đọc phản hồi kết thúc
                    bytesRead = await stream.ReadAsync(responseHeaderBuffer, 0, 4);
                    if (bytesRead < 4)
                    {
                        _logger.LogWarning("Không đọc được phản hồi sau khi gửi tất cả profiles");
                    ...
                    {
                        _logger.LogInformation("Đã gửi tất cả profiles lên server: Thành công={Success}, Lỗi={Error}",
